...
document.addEventListener("DOMContentLoaded", function () {
    ...
    dudas.forEach(duda => {
        duda.addEventListener("click", function (event) {
            ...
            if (mensaje) {
                ...
                mensajeDuda.style.visibility = "visible";  // Hacerlo visible
                mensajeDuda.style.opacity = "1";  // Hacerlo completamente opaco
            }
        });
    });
    ...
    document.addEventListener("click", function (event) {
        if (!mensajeDuda.contains(event.target)) {
            mensajeDuda.style.opacity = "0";  // Hacerlo invisible
            mensajeDuda.style.visibility = "hidden";  // Hacerlo invisible pero mantener el espacio
        }
    });
